...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    //这里就是将事件加入到epoll队列中、这个ngx_handle_read_event函数也是nginx的核心函数
    //放入epoll队列之后、事件执行、epoll_wait获取已经准备好的事件、然后加入执行的队列、由work执行、
    //也就是说work一直在loop、获取连接、执行事件、而请求来了之后就将请求加入到epoll中去\
    //事件是怎么进行下去的呢、就是事件的回调推着一个请求往前走的
    //比如在loop中、把事件加入epoll、事件好了之后执行回调函数、也就是rec上的handler、handler上面的函数再推着请求处理往前走
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    hc = c->data;
    cscf = ngx_http_get_module_srv_conf(hc->conf_ctx, ngx_http_core_module);//(hc->conf_ctx)->srv_conf[ngx_http_core_module.ctx_index]
...
ngx_http_finalize_connection(ngx_http_request_t *r)
{
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);//(r)->loc_conf[ngx_http_core_module.ctx_index]
