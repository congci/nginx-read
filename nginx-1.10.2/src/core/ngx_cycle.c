...
//配置的时候先弄好 sockaddr_in 然后---》socket（）--》连接的hander
...
//会先初始化cycle数据结构、并且解析配置、配置已经解析出来、设置监听
ngx_cycle_t *
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    cycle->conf_ctx = ngx_pcalloc(pool, ngx_max_module * sizeof(void *));//建立指向module配置文件的指针
    ...
    //此处即是解析配置、在解析配置的过程中command对应的函数执行、
...
//初始化配置
    ...
    ccf = (ngx_core_conf_t *) ngx_get_conf(cycle->conf_ctx, ngx_core_module);  //cycle->conf_ctx[ngx_core_module.index]
    ...
    } else if (!ngx_is_init_cycle(old_cycle)) { //!(old_cycle->conf_ctx == NULL)old_cycle的配置有值
