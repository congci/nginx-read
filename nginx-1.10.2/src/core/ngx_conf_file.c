...
//打开文件、如果出错、则记录并且返回
           ...
           )
        {
            ...
            size = ngx_file_size(&cf->conf_file->file.info);//struct--》stat---》st_size 也就是文件大小
...
//循环解析 返回token、并且根据配置执行模块的命令的安装指令、
    for ( ;; ) {
        //依次循环读取、直到读完 一个个token server http { } xxxx;
...
//各个模块的命令
        cmd = cf->cycle->modules[i]->commands;//此模块的所有command
        if (cmd == NULL) {
            continue;
        }
...
//没有{、是开始
            ...
            /* is the directive's argument count right ? 命令的参数是否正确*/
...
ngx_conf_read_token(ngx_conf_t *cf)
{
    ...
    cf->args->nelts = 0; //token个数
    b = cf->conf_file->buffer; //buf
