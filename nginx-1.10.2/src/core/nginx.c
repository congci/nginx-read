...
//core指令 nginx.conf里面的配置
static ngx_command_t  ngx_core_commands[] = {

    //{ sizeof(str) - 1, (u_char *) str }  ngx_core_commands[0][0][1]
    { ngx_string("daemon"),//名字
      NGX_MAIN_CONF|NGX_DIRECT_CONF|NGX_CONF_FLAG,//类型
      ngx_conf_set_flag_slot,//函数指针、一个方法、对应的指令处理的方法 set
      0,//配置数字
      offsetof(ngx_core_conf_t, daemon),//偏移
      NULL },
    ...
    { ngx_string("pid"),
      NGX_MAIN_CONF|NGX_DI  RECT_CONF|NGX_CONF_TAKE1,
...
ngx_module_t  ngx_core_module = {
    NGX_MODULE_V1,  //NGX_MODULE_UNSET_INDEX, NGX_MODULE_UNSET_INDEX,                           \
    NULL, 0, 0, nginx_version, NGX_MODULE_SIGNATURE
    &ngx_core_module_ctx,                  /* module context 配置模块的上下文*/
    ngx_core_commands,                     /* module directives 内核所涉及的指令 */
    NGX_CORE_MODULE,                       /* module type 0x45524F43*/
    NULL,                                  /* init master */
    NULL,                                  /* init module */
    NULL,                                  /* init process */
    NULL,                                  /* init thread */
...
main(int argc, char *const *argv)
{
    ...
    //错误记录初始化、也就是存到ngx_sys_errlist数组中
    ...
    //重要
    //配置和连接初始化、在这里把基本的属性初始化、模块初始化、监听端口
    ...
    ccf = ngx_get_conf(cycle->conf_ctx, ngx_core_module); //(ngx_core_conf_t *) cycle->conf_ctx[ngx_core_module.index]
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (p = inherited, v = p; *p; p++) {
        if (*p == ':' || *p == ';') {
            s = ngx_atoi(v, p - v); //转化为数字、也就是socket
            if (s == NGX_ERROR) {
