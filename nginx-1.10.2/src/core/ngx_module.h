...
//模块基本的数据结构
struct ngx_module_s {
    ngx_uint_t            ctx_index;//模块标示
    ngx_uint_t            index; //在模块的位置
    ...
    void                 *ctx;  //一个指向上下问的指针 里面基本都是回调函数
...
typedef struct {
    ngx_str_t             name;//名字
    void               *(*create_conf)(ngx_cycle_t *cycle);//函数
    char               *(*init_conf)(ngx_cycle_t *cycle, void *conf);//函数
} ngx_core_module_t;
